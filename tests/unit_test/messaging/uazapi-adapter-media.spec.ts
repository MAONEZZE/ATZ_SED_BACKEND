...
function cfg() {
  const b: Record<string, unknown> = { UAZAPI_API_URL: 'https://uaz' };
...
describe('UazapiAdapter.sendMedia', () => {
  ...
  it('posts to /send/media with type/mimetype/file/docName and token header', async () => {
    ...
    const adapter = new UazapiAdapter(cfg() as any);
  ...
  it('throws on non-ok response', async () => {
    ...
    const adapter = new UazapiAdapter(cfg() as any);
    await expect(adapter.sendMedia('t', '+55', 'u', 'image', 'image/png', 'a.png')).rejects.toThrow('Uazapi API error');
  });
});
